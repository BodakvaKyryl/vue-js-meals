<template>
	<div class="max-w-[800px] mx-auto p-8">
		...
		<img :src="meal.strMealThumb" :alt="meal.strMeal" class="max-w-[100%] rounded-2xl mx-auto my-auto" />
		<div class="grid grid-cols-1 py-2 mx-auto text-lg text-center sm:grid-cols-3">
			<div>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2">
			<div>
				<h2 class="mb-2 text-2xl font-semibold text-red-400">Ingredients</h2>
				<ul>
					<li v-for="(ind, index) in 20" :key="index">
						<template v-if="meal[`strIngredient${ind}`] && meal[`strIngredient${ind}`].trim() !== ''">
			...
			<div>
				...
				<ul>
					<li v-for="(ind, index) in 20" :key="index">
						<template v-if="meal[`strMeasure${ind}`] && meal[`strMeasure${ind}`].trim() !== ''">
